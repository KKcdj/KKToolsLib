...
@implementation KKAudioPlayer
+ (instancetype)sharedInstance {
    static KKAudioPlayer *player = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        player = [KKAudioPlayer new];
    });
    return player;
}
...
- (instancetype)init {
    if (self = [super init]) {
        self.playerState = KKAudioPlayerStateStopped;
    }
    return self;
}
...
- (void)play {
    if (self.playerState == KKAudioPlayerStatePlaying) return;
    ...
    // 如果缓冲未完成
    if (self.bufferState != KKAudioBufferStateFinished) {
        self.bufferState = KKAudioBufferStateNone;
...
- (void)pause {
    if (self.playerState == KKAudioPlayerStatePaused) return;
    
    self.playerState = KKAudioPlayerStatePaused;
...
- (void)stop {
    if (self.playerState == KKAudioPlayerStateStoppedBy) return;
    
    self.playerState = KKAudioPlayerStateStoppedBy;
    [self setupPlayerState:self.playerState];
...
- (void)startTimer {
    ...
    self.playTimer = [KKTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(timerAction:) userInfo:nil repeats:YES];
}
...
- (void)startBufferTimer {
    ...
    self.bufferTimer = [KKTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(bufferTimerAction:) userInfo:nil repeats:YES];
}
...
- (void)timerAction:(id)sender {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([self.delegate respondsToSelector:@selector(KKPlayer:currentTime:totalTime:progress:)]) {
            [self.delegate KKPlayer:self currentTime:currentTime totalTime:totalTime progress:progress];
        }
        
        if ([self.delegate respondsToSelector:@selector(KKPlayer:totalTime:)]) {
            [self.delegate KKPlayer:self totalTime:totalTime];
        }
    });
}
...
- (void)bufferTimerAction:(id)sender {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (bufferProgress > 0.9 && buffer >= 1) {
            self.bufferState = KKAudioBufferStateFinished;
            ...
            self.bufferState = KKAudioBufferStateBuffering;
        }
        
        if ([self.delegate respondsToSelector:@selector(KKPlayer:bufferProgress:)]) {
            [self.delegate KKPlayer:self bufferProgress:bufferProgress];
        }
    });
}
...
- (void)setupPlayerState:(KKAudioPlayerState)state {
    if ([self.delegate respondsToSelector:@selector(KKPlayer:statusChanged:)]) {
        [self.delegate KKPlayer:self statusChanged:state];
    }
}
        ...
        _audioStream.onStateChange = ^(FSAudioStreamState state) {
            switch (state) {
                case kFsAudioStreamRetrievingURL:       // 检索url
                    NSLog(@"检索url");
                    weakSelf.playerState = KKAudioPlayerStateLoading;
                    break;
                case kFsAudioStreamBuffering:           // 缓冲
                    NSLog(@"缓冲中。。");
                    weakSelf.playerState = KKAudioPlayerStateBuffering;
                    weakSelf.bufferState = KKAudioBufferStateBuffering;
                    break;
                ...
                case kFsAudioStreamPlaying:             // 播放
                    NSLog(@"播放中。。");
                    weakSelf.playerState = KKAudioPlayerStatePlaying;
                    break;
                case kFsAudioStreamPaused:              // 暂停
                    NSLog(@"播放暂停");
                    weakSelf.playerState = KKAudioPlayerStatePaused;
                    break;
                case kFsAudioStreamStopped:              // 停止
                    
                    // 切换歌曲时主动调用停止方法也会走这里，所以这里添加判断，区分是切换歌曲还是被打断等停止
                    if (weakSelf.playerState != KKAudioPlayerStateStoppedBy && weakSelf.playerState != KKAudioPlayerStateEnded) {
                        NSLog(@"播放停止被打断");
                        weakSelf.playerState = KKAudioPlayerStateStopped;
                    }
                    break;
                case kFsAudioStreamRetryingFailed:              // 检索失败
                    NSLog(@"检索失败");
                    weakSelf.playerState = KKAudioPlayerStateError;
                    break;
                ...
                case kFsAudioStreamPlaybackCompleted:           // 播放完成
                    NSLog(@"播放完成");
                    weakSelf.playerState = KKAudioPlayerStateEnded;
                    break;
                ...
                case kFSAudioStreamEndOfFile:                   // 缓冲结束
                    {
                        ...
                        if (self.bufferState == KKAudioBufferStateFinished) return;
