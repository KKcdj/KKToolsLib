...
//    return uiImage;
    ...
    CGSize size = CGSizeMake(kScreenWidth,kScreenHeight - KK_tabBarFullHeight);
...
/// 用户会员状态
+ (UserVipState)returnUserIsVip{
//    [CurrentVipModel shareInstance].
//    if ([ConfigInitModelSingle shareInstance].model.inapp_vip) {
//        return UserVipStateYes;
//    }
//    if ([DatabaseUtils requestIsRewardedVip]) {
//        return UserVipStateRewards;
//    }
    
    return UserVipStateNo;
}

/// 分享应用
+ (void)shareApp_e2004:(UIViewController *)vc code:(NSString *)code{

//    NSArray *arr = @[[NSURL URLWithString:[NSString stringWithFormat:@"https://itunes.apple.com/cn/app/id%@?mt=8", KKAppID]],KK_LocalizedString(@"我发现一款好用的音乐app, 你也赶紧下载享受好听的音乐吧：")];
    
    NSString *tmpStr = [NSString stringWithFormat:@"【%@】 %@ %@",KK_APP_NAME,[[NSString stringWithFormat:@"%@",KK_LocalizedString(@"赶快下载全球最快并且免费的vpn软件，记得注册填写我的邀请码#num#，首充有奖励哦～")] stringByReplacingOccurrencesOfString:@"#num#" withString:code],[NSString stringWithFormat:@"https://itunes.apple.com/cn/app/id%@?mt=8", KKAppID]];
    NSArray *arr = @[tmpStr];
...
+ (BOOL)isMiniScreenIphone{
    if ([[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 5"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 5c"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 5s"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 6"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 6s"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone SE (1st generation)"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 7"] ||
        [[KKTools returnCurrentDeviceModel] isEqualToString:@"iPhone 8"]
        ) {
        return YES;
    }
