...
NSString * const KKNetworkAccessibilityChangedNotification = @"KKNetworkAccessibilityChangedNotification";

typedef NS_ENUM(NSInteger, KKNetworkType) {
    KKNetworkTypeUnknown ,
    KKNetworkTypeOffline ,
    KKNetworkTypeWiFi    ,
    KKNetworkTypeCellularData ,
};

@interface KKNetworkAccessibility(){
    SCNetworkReachabilityRef _reachabilityRef;
    CTCellularData *_cellularData;
    NSMutableArray *_becomeActiveCallbacks;
    KKNetworkAccessibleState _previousState;
...
@implementation KKNetworkAccessibility
#pragma mark - Public
...
+ (KKNetworkAccessibility *)sharedInstance {
    static KKNetworkAccessibility * instance = nil;
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(void (^)(KKNetworkAccessibleState))block {
...
- (void)setupNetworkAccessibility {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self isSimulator]) {
        ...
        [self notiWithAccessibleState:KKNetworkAccessible];
        return;
    }
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"KKNetworkAccessibilityRunFlag";
...
- (void)cleanNetworkAccessibility {
    ...
    _previousState = KKNetworkChecking;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    KKNetworkAccessibility *networkAccessibility = (__bridge KKNetworkAccessibility *)info;
    if (![networkAccessibility isKindOfClass: [KKNetworkAccessibility class]]) {
        return;
    }
...
- (void)startCheck {
    ...
    if ([self currentReachable]) {
        /* 先用 currentReachable 判断，若返回的为 YES 则说明：
         ...
         此时是有网络访问权限的，直接返回 KKNetworkAccessible
         **/
        return [self notiWithAccessibleState:KKNetworkAccessible];
    }
    ...
    switch (state) {
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(KKNetworkType type) {
                /*  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == KKNetworkTypeCellularData || type == KKNetworkTypeWiFi) {
                    [self notiWithAccessibleState:KKNetworkRestricted];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:KKNetworkUnknown];
                }
            }];
...
- (void)getCurrentNetworkType:(void(^)(KKNetworkType))block {
    if ([self isWiFiEnable]) {
        return block(KKNetworkTypeWiFi);
    }
    KKNetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == KKNetworkTypeWiFi) { // 这时候从状态栏拿到的是 Wi-Fi 说明状态栏没有刷新，延迟一会再获取
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (KKNetworkType)getNetworkTypeFromStatusBar {
    NSInteger type = 0;
    @try {
    ...
    #pragma clang diagnostic pop
        ...
        if (isModernStatusBar) { // 在 iPhone X 上 statusBar 属于 UIStatusBar_Modern ，需要特殊处理
            ...
            return  wifiEnable     ? KKNetworkTypeWiFi :
                    cellularEnable ? KKNetworkTypeCellularData : KKNetworkTypeOffline;
        } else { // 传统的 statusBar
            ...
            return type == 0 ? KKNetworkTypeOffline :
                   type == 5 ? KKNetworkTypeWiFi    : KKNetworkTypeCellularData;
        }
...
- (void)notiWithAccessibleState:(KKNetworkAccessibleState)state {
    if (_automaticallyAlert) {
        if (state == KKNetworkRestricted) {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KKNetworkAccessibilityChangedNotification object:nil];
